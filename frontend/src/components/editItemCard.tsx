import { AspectRatio, Badge, Button, Card, Group, Image, Text, createStyles } from "@mantine/core";
...
export const EditItemCard = ({
    ...
    return (
        <Card p="md" radius="md" className={classes.card}>
            ...
            <Group position="apart" className={classes.spacedContainer} pt="sm">
                <Button onClick={() => onNewAmount(item, item.amount - 1)} variant="filled">
                    -
                </Button>
                <Button onClick={() => onEdit(item)} variant="light">
                    Szerkesztés
                </Button>
                <Button onClick={() => onNewAmount(item, item.amount + 1)} variant="filled">
                    +
